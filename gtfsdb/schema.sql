...
CREATE TABLE
    IF NOT EXISTS routes (
        ...
        text_color TEXT CHECK (
            text_color IS NULL
            OR length (text_color) = 6
        ),
        continuous_pickup INTEGER CHECK (
            continuous_pickup IS NULL
            OR continuous_pickup BETWEEN 0 AND 3
        ),
        continuous_drop_off INTEGER CHECK (
            continuous_drop_off IS NULL
            OR continuous_drop_off BETWEEN 0 AND 3
        ),
...
-- migrate
CREATE TRIGGER IF NOT EXISTS routes_fts_au AFTER
UPDATE ON routes BEGIN
INSERT INTO
    routes_fts (
        routes_fts,
        rowid,
...
INSERT INTO
    stops_fts (rowid, id, stop_name)
VALUES
    (new.rowid, new.id, new.name);
...
CREATE TRIGGER IF NOT EXISTS stops_fts_update_trigger AFTER
UPDATE ON stops BEGIN
...
CREATE TABLE
    IF NOT EXISTS trips (
        ...
        CHECK (
            min_arrival_time IS NULL
            OR max_departure_time IS NULL
            OR min_arrival_time <= max_departure_time
        ),
...
CREATE TABLE
    IF NOT EXISTS stop_times (
        ...
        stop_headsign TEXT,
        pickup_type INTEGER DEFAULT 0 CHECK (
            pickup_type IS NULL
            OR pickup_type BETWEEN 0 AND 3
        ),
        drop_off_type INTEGER DEFAULT 0 CHECK (
...
CREATE INDEX IF NOT EXISTS idx_block_layover_route_service_time ON block_layover (
    route_id,
    service_id,
    layover_end,
    layover_start,
...
CREATE TABLE
    IF NOT EXISTS problem_reports_trip (
        ...
        user_lat REAL CHECK (
            user_lat IS NULL
            OR user_lat BETWEEN -90 AND 90
        ),
        user_lon REAL CHECK (
            user_lon IS NULL
            OR user_lon BETWEEN -180 AND 180
        ),
        user_location_accuracy REAL,
        user_on_vehicle INTEGER CHECK (
            user_on_vehicle IS NULL
            OR user_on_vehicle IN (0, 1)
        ),
...
-- migrate
CREATE INDEX IF NOT EXISTS idx_problem_reports_trip_trip_service ON problem_reports_trip (trip_id, service_date);

-- migrate
CREATE INDEX IF NOT EXISTS idx_problem_reports_trip_created ON problem_reports_trip (created_at);

-- migrate
CREATE INDEX IF NOT EXISTS idx_problem_reports_trip_code ON problem_reports_trip (code);
...
-- migrate
CREATE INDEX IF NOT EXISTS idx_problem_reports_stop_stop ON problem_reports_stop (stop_id);

-- migrate
CREATE INDEX IF NOT EXISTS idx_problem_reports_stop_created ON problem_reports_stop (created_at);

-- migrate
CREATE INDEX IF NOT EXISTS idx_problem_reports_stop_code ON problem_reports_stop (code);
